...
rem Minimal MSVC FFmpeg build (RTSP + H.264 decode): vcvars64 + MSYS2 inherit + clang-cl + 1_build_msvc_rtsp_min.sh
rem Double-click this file, or run it from any Command Prompt.
rem Output: build\install-rtsp-min\  (does not overwrite the full Prism build in build\install)
...
echo.
echo Building FFmpeg MSVC RTSP/H.264-min shared libraries
echo   source: %CD%
echo   msys:   %MSYS_PWD%
echo Launching 1_build_msvc_rtsp_min.sh
echo   Output prefix: build\install-rtsp-min
echo   configure prints a long summary after a few minutes, then make runs.
echo.

"%MSYS2%\usr\bin\bash.exe" -c "cd '%MSYS_PWD%' && ./1_build_msvc_rtsp_min.sh"
echo.
echo FFmpeg RTSP-min build finished with exit code %ERRORLEVEL%
exit /b %ERRORLEVEL%
